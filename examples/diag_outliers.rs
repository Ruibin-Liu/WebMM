...
fn main() {
    let name = env::args().nth(1).expect("usage: diag_outliers <mol_name> [seed]");
    ...
    let seed: i64 = env::args().nth(2).unwrap_or_else(|| "42".into()).parse().unwrap();
    ...
    println!("# name={name} seed={seed} n_atoms={} mmff={e:.4}", mol.atoms.len());
    ...
    for (i, a) in mol.atoms.iter().enumerate() {
        ...
        let c = coords[i];
        println!("A\t{}\t{}\t{:?}\t{:.4}\t{:.4}\t{:.4}", i, a.symbol, hyb, c[0], c[1], c[2]);
    }
    ...
    for i in 0..mol.atoms.len() {
        for a in &adj[i] {
            for b in &adj[i] {
                if a < b {
                    let v1 = [coords[*a][0] - coords[i][0], coords[*a][1] - coords[i][1], coords[*a][2] - coords[i][2]];
                    let v2 = [coords[*b][0] - coords[i][0], coords[*b][1] - coords[i][1], coords[*b][2] - coords[i][2]];
                    let n1 = (v1[0] * v1[0] + v1[1] * v1[1] + v1[2] * v1[2]).sqrt();
                    let n2 = (v2[0] * v2[0] + v2[1] * v2[1] + v2[2] * v2[2]).sqrt();
                    let cos = (v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2]) / (n1 * n2).max(1e-12);
