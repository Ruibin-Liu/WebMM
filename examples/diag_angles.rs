...
fn main() {
    let name = std::env::args().nth(1).unwrap_or_else(|| "caffeine".to_string());
    ...
    println!("{}: embedded {:.1}  SDF {:.1}  diff {:.1}", name, e_emb, e_sdf, e_emb - e_sdf);
    ...
    for &(i, j, k, _t0) in &theta0 {
        ...
        strained.push(((emb - t0d).abs() + (emb - sdf).abs(), i, j, k, emb, sdf, t0d));
    }
    ...
    for &(_, i, j, k, emb, sdf, t0d) in strained.iter().take(8) {
        println!("    {}({})-{}({})-{}({}):  {:5.1} {:5.1} {:6.1}   {:.1}",
            sym(i), i, sym(j), j, sym(k), k, emb, sdf, t0d, (emb-t0d).abs());
    }
}
