...
fn debug_breakdowns() {
    // Cyclohexane
    {
        ...
        assert!(
            (result.final_energy - 2.368812).abs() < 0.1,
            "cyclohexane breakdown: energy {} vs RDKit 2.368812",
            result.final_energy
        );
    ...
    // Phenol
    {
        ...
        let result = optimizer::optimize(&ff, coords, &opts);
        assert!(
            (result.final_energy - 5.713261).abs() < 0.1,
            "phenol breakdown: energy {} vs RDKit 5.713261",
            result.final_energy
        );
    ...
    // Benzene
    {
        ...
        let result = optimizer::optimize(&ff, coords, &opts);
        assert!(
            (result.final_energy - 16.227397).abs() < 0.1,
            "benzene breakdown: energy {} vs RDKit 16.227397",
            result.final_energy
        );
    ...
    // Glycine
    {
        ...
        let result = optimizer::optimize(&ff, coords, &opts);
        assert!(
            (result.final_energy - 20.929656).abs() < 0.1,
            "glycine breakdown: energy {} vs RDKit 20.929656",
            result.final_energy
        );
